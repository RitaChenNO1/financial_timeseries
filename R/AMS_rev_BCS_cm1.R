#1. set data
data_src<-AMS_rev_BU_CUST

#2. filter data
data1<-data_src[which(data_src$BU=="BCS" & data_src$CUST_AMID_LVL_2_CLS_1_CD=="CM1"),]
...
#with seasonal
rev.fit<-arima(ts_rev,order=c(0,0,0), seasonal=list(order=c(0,0,1), period=3))
...
logrev.fit<-arima(log(ts_rev),order=c(2,1,3), seasonal=list(order=c(1,1,0), period=3))
plot.ts(logrev.fit$residuals)
#summary(logrev.fit)
logrev.fit
logrev.fit<-arima(log(ts_rev),order=c(0,1,0))
plot.ts(logrev.fit$residuals)
#summary(logrev.fit)
logrev.fit
...
write.table(fall, file = "C:/WORK/EGI/data/forecast_ams_cp_rev_bcs_cm1_000_seasonal001.csv",quote = TRUE, sep = ",",row.names = FALSE)
