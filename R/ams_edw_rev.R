...
#data1<-data_src[which(as.integer(row.names(data1))>=11),]
data0<-data1[which(as.integer(row.names(data1))<=len-2),] #remove the not mature data
#len<-dim(data0)[1]
...
ts_rev <- ts(fit.training$edw_rev, start=c(2014,02), frequency=12)
